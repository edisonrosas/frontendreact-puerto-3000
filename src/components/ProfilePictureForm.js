...
class ProfilePictureForm extends PureComponent {
  ...
  handleUpload = (event) => {
    ...
    const { imgSrc, uuidName, imgFile } = this.state;
    if (imgSrc) {
      const canvasRef = this.imagePreviewCanvasRef.current;
  ...
  verifyFile = (files) => {
    if (files && files.length > 0) {
      ...
      if (!acceptedFileTypesArray.includes(currentFileType)) {
        dispatch(
          alertActions.error(
            "Este archivo no está permitido. Solo se permite imagenes."
          )
        );
       ...
       // console.log(currentFile)
        ...
        myFileItemReader.addEventListener(
          ...
          () => {
            ...
            const urlImg = "https://redsocialviajes.s3.amazonaws.com/perfil/"+uuidName;
            console.log(urlImg)
